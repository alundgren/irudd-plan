...
import type { AssetDescriptor, Decision, Plan, SharedContext, WorkItem } from "../contract/plan.js";
...
function uniqueIds<T extends { readonly id: string }>(
  ...
  for (const record of records) {
    ...
    if (seen.has(record.id)) {
      throw new PlanError("DUPLICATE_ID", `Duplicate id: ${record.id}`, { id: record.id, kind });
    }
    seen.add(record.id);
...
function requireReference<T>(map: ReadonlyMap<string, T>, id: string, source: string): T {
  const value = map.get(id);
  if (value === undefined) {
    throw new PlanError("REFERENCE_MISSING", `${source} references missing id: ${id}`, {
      id,
      source,
    });
  }
...
function visitContexts(
  ...
  if (visiting.has(id)) {
    throw new PlanError("REFERENCE_CYCLE", `Required context cycle includes: ${id}`, { id });
  }
...
function requireAvailableAsset(
  ...
  if (!asset.available) {
    throw new PlanError("ASSET_UNAVAILABLE", `${source} requires unavailable asset: ${id}`, {
      id,
      source,
    });
  }
...
export function validatePlan(plan: Plan): PlanIndex {
  ...
  for (const item of plan.items) {
    ...
    for (const id of item.relatedItemIds) requireReference(items, id, `item ${item.id}`);
  }
  ...
  for (const context of plan.contexts) {
    ...
    for (const id of context.assetIds) requireAvailableAsset(assets, id, `context ${context.id}`);
  }
  ...
  for (const id of contexts.keys()) visitContexts(id, contexts, new Set(), visited);
...
export function collectRequiredPacket(plan: Plan, itemId: string): RequiredPacketContent {
  const index = validatePlan(plan);
  const item = index.items.get(itemId);
  if (item === undefined) {
    throw new PlanError("ITEM_NOT_FOUND", `Work item not found: ${itemId}`, { itemId });
  }
  ...
  return {
    item,
    contexts: [...contextIds].sort().map((id) => requireReference(index.contexts, id, item.id)),
    decisions: [...selectedDecisions].sort((a, b) => a.id.localeCompare(b.id)),
    assets: [...assetIds].sort().map((id) => requireReference(index.assets, id, item.id)),
  };
}
