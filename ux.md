...
Fit shows summaries at every scale. Manual zoom shows summaries below 45%. Hidden full content stays mounted but does not reserve row height. Item titles and the epic goal enlarge as the camera zooms out, targeting 16px on screen up to a 100px canvas font limit. The Goal label scales too, targeting 13.6px. Titles wrap fully, and measured rows accommodate their height. Short goals remain secondary to item titles. Every required visual has a summary link and image thumbnail where supported; selecting it opens the full reference. Each reference remains reachable. Caption previews stop after three lines; hover or opening the reference reveals the complete caption. Summary content determines frame height. Fit can shrink below a readable text size to include large plans and distant canvas comments. The searchable chooser provides readable navigation at any scale.
...
The bottom toolbar retains Comment and Pan, C/V shortcuts, zoom out/in, a percentage button that resets to 100%, and Fit. Comment is the default and summaries navigate to full content. C/V shortcuts work while ordinary buttons or links have focus, including after choosing a work item. They preserve focus and ignore editable content, menus, dialogs, composition and modified keys. Comment clicks on full text, drawings or blank canvas space open a focused dialog with editable About and Your feedback fields. Only Add comment saves a new note. Cancel and Escape leave notes and counts untouched and restore focus. A browser unload warning protects typed new feedback and unsaved edits from accidental reload or tab close. Clean dialogs do not warn. Edit uses the same dialog; Save comment changes wording while keeping the original target and revision.
...
controls. Returning preserves the item camera and unsent answer drafts. Public
